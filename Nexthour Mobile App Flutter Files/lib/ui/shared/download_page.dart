...
class _DownloadPageState extends State<DownloadPage> with TickerProviderStateMixin {
  ...
  void _showDialog(task) {
    var userProfileProvider = Provider.of<UserProfileProvider>(context).userProfileModel;
    getAllScreens();
    var downCount;
  ...
  Widget _buildActionForTask(TaskInfo task) {
    ...
    if (task.status == DownloadTaskStatus.undefined) {
      ...
      int progress = 100 ;
      createTodo(task, task.name, dFileName, widget.videoDetail.type,
  ...
  void _delete(TaskInfo task) async {
    ...
    var raw = await cdb.delete(
      ...
      where: "movie_id = ? AND vtype = ?", whereArgs: ['${widget.videoDetail.id}', '${widget.videoDetail.type}'],
    );
    setState(() {});
  }
  ...
  void _requestDownload(TaskInfo task) async {
    ...
    int progress = 0;
    createTodo(task, task.name, dFileName, widget.videoDetail.type, widget.videoDetail.id, task.taskId, progress, task.hdLink);
  }
  ...
  void _requestDownload360(TaskInfo task) async {
    ...
    int progress = 0;
    createTodo(task, task.name, dFileName, widget.videoDetail.type, widget.videoDetail.id, task.taskId, progress, task.link360);
  }
  void _requestDownload480(TaskInfo task) async {
    ...
    int progress = 0;
    createTodo(task, task.name, dFileName, widget.videoDetail.type, widget.videoDetail.id, task.taskId, progress, task.link480);
  }
  void _requestDownload720(TaskInfo task) async {
    ...
    int progress = 0;
    createTodo(task, task.name, dFileName, widget.videoDetail.type, widget.videoDetail.id, task.taskId, progress, task.link720);
  }
  void _requestDownload1080(TaskInfo task) async {
    ...
    int progress = 0;
    createTodo(task, task.name, dFileName, widget.videoDetail.type, widget.videoDetail.id, task.taskId, progress, task.link1080);
  }
  ...
  Future<bool> _openDownloadedFile(TaskInfo task) async {
    var response = await cdb.query(
      DatabaseCreator.todoTable,
      where: "movie_id = ? AND vtype = ?",
      whereArgs: [widget.videoDetail.id, widget.videoDetail.type],
    );
...
//  Download text
  ...
  void _showMsg(userProfileProvider) {
    if (userProfileProvider.paypal.length == 0 || userProfileProvider.stripe.length == null) {
