...
//  Creating stripe subscription form the customer using customer Id and plan.
  Future<String> _createSubscription(customerStripeId, cardid, cardtype, cardBrand, cardLast4, stripePass,  planId) async {
    ...
    if(widget.couponCode != ''){
      var stripeUri = "https://api.stripe.com/v1/customers/"+"$customerStripeId"+"/subscriptions?plan="+"$planId"+"&quantity=1&default_source="+"$cardid"+"&coupon="+"${widget.couponCode}";
      ...
      subscriptionResponse = await http.post(
          Uri.encodeFull("https://api.stripe.com/v1/customers/"+"$customerStripeId"+"/subscriptions?plan="+"$planId"+"&quantity=1&default_source="+"$cardid"),
          headers: {
            // ignore: deprecated_member_use
            HttpHeaders.AUTHORIZATION: "Bearer $stripePass"
          });
              ...
              }else
              {
                if(stripeKey == null){
                  ...
                  if(stripeCustomerId != null){
                    ...
                    _createCustomer(userDetails.user.name, userDetails.user.email, stripePass, planList[widget.index].id);
                  }
                  showSuccessDialog();
                }
